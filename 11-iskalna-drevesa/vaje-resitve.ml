...
let leaf (x : 'a) : 'a bin_tree =
    Node (Empty, x, Empty)

let test_tree : int bin_tree =
...
let rec mirror (tree : 'a bin_tree) : 'a bin_tree =
    match tree with
    | Empty -> Empty
...
let rec map_tree (f : 'a -> 'b) (tree : 'a bin_tree) : 'b bin_tree =
    match tree with
    | Empty -> Empty
...
let rec list_of_tree (tree : 'a bin_tree) : 'a list =
    match tree with
    | Empty -> []
...
let is_bst (tree : 'a bin_tree) : unit = ()
...
let rec insert (n : 'a) (tree : 'a bin_tree) : 'a bin_tree =
    match tree with
    | Empty -> Node (Empty, n, Empty)
    | Node (lt, x, rt) ->
        if n = x then tree
        else
            if n < x then Node (insert n lt, x, rt)
            else Node (lt, x, insert n rt)

let rec member (n : 'a) (tree : 'a bin_tree) : bool =
    match tree with
    | Empty -> false
    | Node (lt, x, rt) ->
        if n = x then true
...
let rec member2 (n : 'a) (tree : 'a bin_tree) : bool =
    match tree with
    | Empty -> false
    | Node (lt, x, rt) ->
        if n = x then true
        else (member2 n lt) || (member2 n rt)
...
let succ (tree : 'a bin_tree) : 'a option =
    match tree with
    | Empty -> None
    | Node (_, _, rt) ->
        match rt with
...
let pred (tree : 'a bin_tree) : 'a option =
    match tree with
    | Empty -> None
    | Node (lt, _, _) ->
        match lt with
